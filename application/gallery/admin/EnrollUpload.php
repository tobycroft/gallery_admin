...
class EnrollUpload extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = EnrollUploadModel::alias("a")->field('*,a.id as id')->leftJoin(["g_enroll" => "b"], "a.enroll_id=b.id")->where($map)->order($order)->paginate();
    ...
    public function add()
    {
        ...
        return ZBuilder::make('form')
            ...
            ->addFormItems([ // 批量添加表单项
                ['select', 'uid', '用户手机', '', UserModel::column('id,phone')],
                ['select', 'enroll_id', '报名人', '', EnrollModel::column('id,name')],
                ['text', 'title', '标题', ''],
                ['textarea', 'content', '内容', ''],
                ['image', 'attachment', '上传图片', ''],
                ['text', 'teacher_name', '教师名称', ''],
                ['text', 'teacher_phone', '教师电话', ''],
            ])
            ->fetch();
    }
