...
class EnrollFree extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = EnrollModel::alias('a')->field('a.*,b.rating,b.attachment')->leftJoin(['g_enroll_upload' => 'b'], 'b.enroll_id=a.id')->where($map)->where("source", "free")->order($order)->paginate()->each(function ($item) {
//            $item['attachment'] = '';
//            $up = EnrollUploadModel::where('enroll_id', $item['id'])->findOrEmpty();
//            if (!$up->isEmpty()) {
//                $item['attachment'] = $up['attachment'];
...
//            ->setSearchArea([['select', 'school_id', '学校id', "", "", $school], ['text', 'year', '入学年份'], ['text', 'grade', '年级'], ['text', 'class', '班级'],])
            ->setSearchArea([
                ...
                ['select', 'remark', '备注长度', 'neq', '', ['null' => '不为空', '' => '为空']],
                ['select', 'remark', '备注长度', '=', '', ['null' => '不为空', '' => '为空']],
            ])
            ->addTopButton("add")
            ->addTopButton("export")
            ->setPageTitle('列表')
